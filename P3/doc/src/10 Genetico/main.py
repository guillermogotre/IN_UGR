...
def validacion_cruzada(modelo, X, y, cv, print, stop_at=3):
    y_test_all = []
    v_acc = []
    i = 0
...
def buildCls(c, c_transf):
    p = [c_transf[i](c[i]) for i in range(c.size)]
    p = [0,0]+p
    #clf1 = xgb.XGBClassifier(n_estimators=p[1], max_depth=p[0], n_jobs=8, objective='multi:softmax')
...
def train(X,y,n_estimators=200, do_print=True, col_names = None):
    ...
    pop[0] = [0.15, 11 / 81, 0.9999999999, 800 / 1300, 400 / 1100, 10 / 21, 0, 0]
    pop[1] = [0.01558524, 0.68074568, 0.48268868, 0.58841161, 0.66274702, 0.23435038, 0.42915351, 0.62010538]
    pop[2] = [0.13717363, 0.34505228, 0.69935156, 0.58440819, 0.359322, 0.63282816, 0.07534677, 0.04265692]
    pop[3] = [0.26326847, 0.44902203, 0.8003606, 0.6581693, 0.53211626, 0.60003022, 0.07926144, 0.0560459]
    pop[4] = [0.05109112, 0.5092589, 0.48277801, 0.55491262, 0.35436072, 0.62106348, 0.63720805, 0.40584696]
    pop_ft = np.zeros((P_SIZE))

    def valida(x):
        clf = buildCls(x, p_transf)
        _, _, v_acc = validacion_cruzada(clf, X, y, skf, cprint, 2)
        h = np.average(v_acc)
        return h
...
def main():
    ...
    clf, cv_acc = train(X, y, 1000, False, col_names)
