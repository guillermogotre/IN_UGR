...
INTERSTING_SMALL_2 = ['lga','payment_type', 'source', 'waterpoint_type']
TODROP_KEYS = ["recorded_by","payment","quality_group","quantity_group"]
TODROP_KEYS2 =  ["recorded_by","payment","quality_group","quantity_group",'extraction_type_group','extraction_type_class','num_private',"waterpoint_type_group","source_class", 'management_group', 'permit', 'public_meeting','source_type','basin','region','region_code','month_n','management']
...
def oneHot(tr,tst):
    ...
    X_all = pd.concat([tr, tst], axis=0)[keys]
...
def preprocess(data_x, data_y, data_x_tst,w2vsize=6,w2vwindow=12):
    ...
    #to_drop = list(filter(lambda x: x in data_x.columns, TODROP_KEYS2))
    to_drop = list(filter(lambda x: x in data_x.columns, TODROP_KEYS))

    data_x.drop(labels=to_drop, axis=1, inplace=True)
    data_x_tst.drop(labels=to_drop, axis=1, inplace=True)
...
def nooverfit(cv,tr):
    alpha = 0.9
    f1 = cv
    #f2 = np.power(1 - (tr - cv) / (1 - cv),1/10)
    f2 = 1 - (tr - cv) / (1 - cv)
    f2 = 1 / f2 if f2 > 1 else f2
    f2 = np.power(f2,1/5)

    return f1 * alpha + f2 * (1-alpha)

def validacion_cruzada(modelo, X, y, cv, print, msk, stop_at=10):
    y_train_all = []
    y_test_all = []
    prob_all = np.array([]).reshape((0,3,len(modelo.clfs)))
    prob_test = np.array([]).reshape((0,3,len(modelo.clfs)))
    v_acc = []
    all_v_acc = []
    i = 0
    for train, test in cv.split(X, y):
        t = time.time()
        mskd_train = np.intersect1d(np.where(msk)[0],train)
        modelo = modelo.fit(X[mskd_train],y[mskd_train])
        tiempo = time.time() - t
        y_pred, prob = modelo.predict(X[test],return_prob=True)
        #prob_pred = modelo.
        all_y_pred, prob_t = modelo.predict(X,return_prob=True)
        acc = accuracy_score(y[test],y_pred)
        all_acc = accuracy_score(y,all_y_pred)
        v_acc += [acc]
        all_v_acc += [nooverfit(acc,all_acc)]
        print("Score: {:.4f}/{:.4f}, tiempo: {:6.2f} segundos".format(acc,all_acc, tiempo))
        y_train_all = np.concatenate([y_train_all,y[test]])
        y_test_all = np.concatenate([y_test_all,y])
        prob_all = np.vstack([prob_all,prob])
        prob_test = np.vstack([prob_test,prob_t])
        i+= 1
        if i >= stop_at:
            break
    ...
    return modelo, (prob_all,y_train_all,prob_test,y_test_all), v_acc, all_v_acc
...
def oneof(lst):
    return lambda x: lst[np.min([len(lst)-1,int(x*len(lst))])]

def cruce(p1,p2):
    h = np.vstack((p1,p2))
    lims = np.vstack((np.min(h,axis=0),np.max(h,axis=0))).transpose()
    alpha = 0.15
    def genTwo(x):
        minx = x[0]
        maxx = x[1]
        ...
        minx = minx-m
        maxx = maxx+m
        return np.random.random(2)*(maxx-minx)+minx
    h = np.clip(np.apply_along_axis(genTwo,1,lims).transpose(),0,1-1e-20)
    return h[0], h[1]

def muta(p):
    i = np.random.randint(p.size)
    p[i] = np.random.random(1)
    return p

def getParams(c, c_transf):
    return [c_transf[i](c[i]) for i in range(len(c))]

def buildCls(c, c_transf):
    p = getParams(c, c_transf)
    clf1 = xgb.XGBClassifier(n_estimators=p[1], max_depth=p[0], n_jobs=NPROC, objective='multi:softmax',tree_method='gpu_hist',gpu_id=GPU_ID)
    ...
    #clf = CustomClassifier(clf2, clf3)

    return clf

#def reemplazo(pop, pop_ft, [h1,h2], [h1_ft,h2_ft])
def reemplazo(pop, pop_ft, hs, hs_ft):
    cnt = len(pop)
    s = sorted(zip(pop_ft+hs_ft,list(pop)+hs),key=lambda x: x[0],reverse=True)
    pop = np.array([x[1] for x in s])
    pop_ft = [x[0] for x in s]
    return np.array(pop)[:cnt], pop_ft[:cnt]

def valida(p):
    target = np.array([0.1,0.9,0.1,0.9,0.1,0.9,0.1,0.9,0.1,0.9])
    return 1-np.sqrt(np.sum(np.power(p-target,2)))/10
...
best_params = [0.20255799, 0.64062604, 0.53709839, 0.80093068, 0.67084769, 0.85401771, 0.24370506, 0.80628683, 0.04472441,
         0.01276028]
...
#[0.00554492 0.48552373 0.52151246 0.67715451 0.66806655 0.70721367 0.1257645  0.42139806]
def train(X,y,n_estimators=200, do_print=True, col_names = None):
    ...
    print("------ Classifiers...")
    # max_depth 3:20, n_estimators 50:800
    p1 = [intbetween(3,21),intbetween(50,800)]
    clf1 = xgb.XGBClassifier(n_estimators = 500, max_depth=14, n_jobs=NPROC, objective='multi:softmax', tree_method='gpu_hist')

    # boosting_type=[gbdt,dart,goss], num_leaves=20:100, max_depth=5:20, n_estimators= 200:1200
    p2 = [oneof(['gbdt','dart','goss']),intbetween(20,101),intbetween(5,100),intbetween(200,1500)]
    clf2 = lgb.LGBMClassifier(objective='multiclass', n_estimators=1000, n_jobs=NPROC)

    p3 = [intbetween(100,1200),intbetween(10,31),intbetween(2,20),intbetween(1,20)]
    # n_estimators=200:1200, max_depth=10:30, min_samples_split=0.01:0.5, min_samples_leaf=0.1:0.3
    clf3 = RandomForestClassifier(n_estimators=500,max_depth=20,n_jobs=NPROC)

    P_SIZE = 6
    p_transf = p1+p2+p3

    pop = np.random.random((P_SIZE,len(p_transf)))
    pop[0] = [0.20255799, 0.64062604, 0.53709839, 0.80093068, 0.67084769, 0.85401771, 0.24370506, 0.80628683, 0.04472441, 0.01276028]
    pop[1] = [0., 0.6, 0.17916901, 0.80042335, 0.81231054, 0.64825269, 0.59849025, 0.66351866, 0.04397433, 0.01262955]
    pop[2] = [0., 0.60180991, 0.15148986, 0.80077069, 0.66193684, 0.73077631, 0.31368961, 0.75284609, 0.04476012, 0.01489641]
    pop[3] = [0, 450 / 750, 0.15, 11 / 81, 0.9999999999, 800 / 1300, 400 / 1100, 10 / 21, 0, 0]
    pop[4] = [0.14116079, 0.41917303, 0.57645603, 0.16122472, 0.2850101,  0.77515019, 0.66128777, 0.46194947, 0.04786077, 0.58215404]

    pop_ft = np.zeros((P_SIZE))

    p = [0.20255799, 0.64062604, 0.53709839, 0.80093068, 0.67084769, 0.85401771, 0.24370506, 0.80628683, 0.04472441,
         0.01276028]
    # clf = buildCls(p, p_transf)
    # clf = CustomClassifier(clf2)
    # mask_path = "noise_mask.npz"
    # if os.path.isfile(mask_path):
    #     with open(mask_path, 'rb') as ifile:
    #         msk = np.load(ifile)
    # else:
    #     with open(mask_path, 'wb') as ofile:
    #         msk = cleanNoise(clf, X, y)
    #         np.save(ofile, msk)
    msk = np.zeros(X.shape[0],dtype=np.bool)+1
    ...
    ####
    import pickle
    acc_path = "saved_acc.json"
    if os.path.isfile(acc_path):
        with open(acc_path, 'rb') as ifile:
            acc_dict = pickle.load(ifile)
    else:
        acc_dict = {}

    def saveAccDict(acc_dict):
        with open(acc_path, 'wb') as ofile:
            pickle.dump(acc_dict,ofile,pickle.HIGHEST_PROTOCOL)

    def valida(x):
        nonlocal p_transf,X,y,skf,cprint,msk
        xstr = str(getParams(x,p_transf))
        if xstr in acc_dict:
            #print("HIT!")
            #v_acc, all_v_acc, nofit_v_acc = acc_dict[xstr]
            acc, acc_test, h, v = acc_dict[xstr]
            #h = np.average(v_acc)+np.average(all_v_acc)
            return acc
        else:
            clf = buildCls(x, p_transf)
            modelo, (prob,y_prob,prob_test,y_prob_test),v_acc, all_v_acc = validacion_cruzada(clf, X, y, skf, cprint, msk, 2)
            #h = np.average(v_acc)
            #DifEvol params
            acc, v = bestWeight(prob,y_prob)
            acc_test = getPondAcc_text(prob_test,y_prob_test,v)
            #h = np.average(v_acc)+np.average(all_v_acc)
            h = np.sqrt(acc*acc_test)
            #acc_dict[xstr] = (v_acc, all_v_acc, nofit_v_acc)
            acc_dict[xstr] = (acc, acc_test, h, v)
            print(h,acc,acc_test)
            saveAccDict(acc_dict)
            return acc
    
    pop_ft = [valida(x) for x in pop]
    ...
    def cb(x,convergence):
        nonlocal i
        i += 1
        print("Generacion {}: {:.5f} | {}".format(i,valida(x),convergence))
        print(x)
    ...
    for i in range(MAX_ITERS):
        ...
        p = np.random.randint(0,P_SIZE,2)
        while p[0] == p[1]:
            p = np.random.randint(0,P_SIZE,2)

        # 2 Cruces
        h1,h2 = cruce(pop[p[0]],pop[p[1]])
        ...
        # Eval hijos
        # clf1 = buildCls(h1,p_transf)
        # _, _, v_acc = validacion_cruzada(clf1, X, y, skf, cprint)
        # #v_acc = valida(h1)
        # h1_ft = np.average(v_acc)
        h1_ft = valida(h1)

        # clf2 = buildCls(h2,p_transf)
        # #_, _, v_acc = validacion_cruzada(clf2, X, y, skf, cprint)
        # v_acc = valida(h2)
        # h2_ft = np.average(v_acc)
        h2_ft = valida(h2)

        # Remplazo
        pop, pop_ft = reemplazo(pop, pop_ft, [h1,h2], [h1_ft,h2_ft])
   ...
   # #pop[0] = [0.20528849, 0.63246539, 0.51678733, 0.83186723, 0.64508234, 0.88954159, 0.24369816, 0.84524633, 0.08769156, 0.02010075]
   # pop[0] = [0., 0.60180991, 0.15148986, 0.80077069, 0.66193684, 0.73077631, 0.31368961, 0.75284609, 0.04476012, 0.01489641]

   # #lgbm, y_test_lgbm, v_acc = validacion_cruzada(clf, X, y, skf, cprint, msk)
   # #v_acc = [0.8151, 0.8192, 0.8210, 0.8178, 0.8180]
   # #sorted(list(zip(clf.feature_importances_,range(231))),reverse=True)

   # #clf = clf.fit(X, y, feature_name=list(col_names))
   # clf = buildCls(pop[0],p_transf)

    lgbm, y_test_lgbm, v_acc, all_v_acc = validacion_cruzada(clf, X, y, skf, cprint, msk)

    clf = clf.fit(X[msk], y[msk])
