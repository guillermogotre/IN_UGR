...
def validacion_cruzada(modelo, X, y, cv, print, msk, stop_at=10):
    ...
    return modelo, y_test_all, v_acc, all_v_acc
...
def oneof(lst):
    return lambda x: lst[int(x*len(lst))]
...
def buildCls(c, c_transf):
    p = [c_transf[i](c[i]) for i in range(len(c))]
...
def train(X,y,n_estimators=200, do_print=True, col_names = None):
    ...
    pop[2] = [0, 450 / 750, 0.17558292, 0.31487049, 0.69838505, 0.42589426, 0.40468874, 0.52577021, 0.0854134, 0.01384353]
    ...
    def valida(x):
        ...
        lgbm, y_test_lgbm, v_acc, all_v_acc = validacion_cruzada(clf, X, y, skf, cprint, msk, 2)
        #h = np.average(v_acc)
        h = np.average(all_v_acc)
        return h
